...
# ------------------- OCR Setup ------------------
# Указываем путь к бинарнику Tesseract внутри проекта
pytesseract.pytesseract.tesseract_cmd = os.path.join(os.path.dirname(__file__), "tesseract_bin/tesseract")
# -----------------------------------------------

# ------------------- Discord Setup ------------------
TOKEN = os.getenv("DISCORD_TOKEN")
GUILD_ID = 1376255869578252358  # <- вставь ID своего сервера
ROLE_NAME = "Verified"          # роль, которую выдаём после верификации
LOG_CHANNEL_NAME = "log"        # канал для логов
VERIFICATION_CHANNEL_NAME = "verification"  # канал, куда присылают скриншоты
...
def extract_game_info(image_bytes):
    ...
    # Ищем игровой ID (цифры) и тег [G13E]
    import re
...
async def on_member_join(member):
    # Проверяем базу данных, выдаём роль, если пользователь уже верифицирован
...
async def on_message(message):
    if message.author.bot:
        return
    if str(message.channel) != VERIFICATION_CHANNEL_NAME:
        return
